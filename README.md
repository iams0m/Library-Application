...
### ✔️ 빌드와 실행
  ...
  * `./gradlew build -x test` : 테스트는 생략하고 프로젝트 빌드
...
### ✔️ 종료되지 않는 실행
  ...
  * `tail` : 현재 파일의 끝부분 출력
    * `tail -f` : 파일 내용물이 추가되면 추가된 내용물도 지속적으로 보여줌 

### ✔️ 가비아를 이용한 도메인 구입
  <p><가비아>라는 사이트를 통해 도메인을 구입하고, 우리가 빌린 EC2 서버를 연결할 수 있다.</p>
  <p>도메인을 이용하게 되면,</p>

  * IP 주소를 입력하는 것이 아닌, 우리가 설정한 도메인 이름으로 접속할 수 있게 된다!

### 📚 Section 7 학습 내용
#### 1️⃣ EC2에 접속하는 방법을 알아보고, 리눅스 명령어를 학습했다.
#### 2️⃣ 지금까지 개발한 서버를 배포하기 위해 리눅스를 통해 환경을 세팅하고, 실제 배포를 진행했다.
#### 3️⃣ foreground에서 동작하고 있는 서버를 background로 동작할 수 있게 변경했다.
#### 4️⃣ 복잡한 IP 주소 대신 도메인 이름으로 접속하는 방법을 적용해 보았다.
</details>
